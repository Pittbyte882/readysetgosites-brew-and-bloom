  ...
  try {
    ...
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html:    `<p style="font-family:sans-serif"><strong>${name ?? 'Someone'}</strong> subscribed: <a href="mailto:${email}">${email}</a></p>`,
      }),
    })
        ...
        html: `
          <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#4A3728">
            <div style="background:linear-gradient(135deg,#FDF2F8,#F0FDF4);padding:32px 40px;text-align:center">
              ...
              <p style="color:#7A6B5A;font-style:italic">${siteConfig.tagline}</p>
            </div>
            <div style="padding:40px;background:#FFFBF5;border:1px solid #E8DFD3;text-align:center">
              ...
              <p style="color:#7A6B5A;line-height:1.8;max-width:400px;margin:0 auto">
                You're now part of our blooming community. Expect seasonal menu updates, 
                event invitations, and the occasional surprise from our kitchen.
              </p>
              <div style="margin:32px 0">
                <a href="${siteConfig.seo.siteUrl}/menu" style="display:inline-block;background:#E8A4B8;color:#4A3728;padding:12px 32px;font-size:13px;letter-spacing:0.12em;text-transform:uppercase;text-decoration:none;border-radius:50px">
                  Explore Our Menu
                </a>
              </div>
