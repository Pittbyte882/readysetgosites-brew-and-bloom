...
function DrinkCard({ item, index }: { item: typeof siteConfig.menuItems[0]; index: number }) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-primary/20 to-secondary/20">
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-1">
          ...
          {item.tags.some(t => t.includes("Only")) && (
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t">
          ...
          {item.tags.length > 0 && (
            <div className="flex gap-1">
              {item.tags.filter(t => ["Vegan", "Gluten-Free", "Dairy-Free"].includes(t)).slice(0, 2).map((t) => (
                <span
                  key={t}
...
export function FeaturedDrinks() {
  ...
  // Pull featured items directly from siteConfig — control everything from config/site.ts
  const featuredItems = siteConfig.featuredDrinkIds
    .map(id => siteConfig.menuItems.find(item => item.id === id))
    .filter(Boolean) as typeof siteConfig.menuItems
